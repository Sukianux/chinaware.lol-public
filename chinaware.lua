...
local whitelistedBoxKeys = {Enum.KeyCode.Zero,Enum.KeyCode.One,Enum.KeyCode.Two,Enum.KeyCode.Three,Enum.KeyCode.Four,Enum.KeyCode.Five,Enum.KeyCode.Six,Enum.KeyCode.Seven,Enum.KeyCode.Eight,Enum.KeyCode.Nine};
local keyNames = {[Enum.KeyCode.LeftControl]="LCTRL",[Enum.KeyCode.RightControl]="RCTRL",[Enum.KeyCode.LeftShift]="LSHIFT",[Enum.KeyCode.RightShift]="RSHIFT",[Enum.UserInputType.MouseButton1]="MB1",[Enum.UserInputType.MouseButton2]="MB2",[Enum.UserInputType.MouseButton3]="MB3"};
...
do
	utility.Connection = function(self, signal, func)
		local c = signal:Connect(func);
		table.insert(library.connections, c);
		return c;
	end;
	utility.Instance = function(self, class, properties)
	...
	utility.HasProperty = function(self, obj, prop)
		return ({(pcall(function()
			local a = obj[prop];
		end))})[1];
	end;
	utility.ToRGB = function(self, c3)
		return c3.R * 255, c3.G * 255, c3.B * 255;
	end;
	utility.AddRGB = function(self, a, b)
	...
	utility.ConvertNumberRange = function(self, val, oldmin, oldmax, newmin, newmax)
		return (((val - oldmin) * (newmax - newmin)) / (oldmax - oldmin)) + newmin;
	end;
	utility.UDim2ToVector2 = function(self, udim2, vector2)
		local x, y;
	...
	utility.Lerp = function(self, a, b, c)
		return a + ((b - a) * c);
	end;
	utility.Tween = function(self, obj, prop, val, time, direction, style)
	...
	utility.DetectTableChange = function(self, indexcallback, newindexcallback)
	...
	utility.MouseOver = function(self, obj)
	...
	utility.GetHoverObject = function(self)
		local objects = {};
	...
	utility.Draw = function(self, class, properties)
		local blacklistedProperties = {"Object","Children","Class"};
		local drawing = {Object=Drawing.new(class),Children={},ThemeColor="",OutlineThemeColor="",ThemeColorOffset=0,OutlineThemeColorOffset=0,Parent=nil,Size=newUDim2(0, 0, 0, 0),Position=newUDim2(0, 0, 0, 0),AbsoluteSize=newVector2(0, 0),AbsolutePosition=newVector2(0, 0),Hover=false,Visible=true,MouseButton1Down=library.signal.new(),MouseButton2Down=library.signal.new(),MouseButton1Up=library.signal.new(),MouseButton2Up=library.signal.new(),MouseEnter=library.signal.new(),MouseLeave=library.signal.new(),Class=class};
		drawing.Update = function(self)
		...
		drawing.UpdateChildren = function(self)
			for i, v in next, drawing.Children do
				v:Update();
			end
		end;
		drawing.GetDescendants = function(self)
			local descendants = {};
			local function a(t)
...
library.init = function(self)
	...
	self.SetTheme = function(self, theme)
		for i, v in next, theme do
			self.theme[i] = v;
		end
		self.UpdateThemeColors();
	end;
	self.GetConfig = function(self, name)
	...
	self.LoadConfig = function(self, name)
	...
	self.SaveConfig = function(self, name)
	...
	utility:Instance("ImageButton", {Parent=screenGui,Visible=true,Modal=true,Size=UDim2.new(1, 0, 1, 0),ZIndex=9999999999,Transparency=1});
	...
	self.SetOpen = function(self, bool)
		self.open = bool;
	...
	self.UpdateThemeColors = function()
		for _, v in next, library.drawings do
	...
	self.SendNotification = function(self, message, time, color)
		time = time or 5;
		...
		do
			...
			notification.holder = utility:Draw("Square", {Position=newUDim2(0, 0, 0, 75),Transparency=0});
			notification.background = utility:Draw("Square", {Size=newUDim2(1, 0, 1, 0),Position=newUDim2(0, -500, 0, 0),Parent=notification.holder,ThemeColor="Background",ZIndex=z});
			notification.border1 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Border 2",Parent=notification.background,ZIndex=(z - 1)});
			objs.border2 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Border 3",Parent=objs.border1,ZIndex=(z - 2)});
			notification.gradient = utility:Draw("Image", {Size=newUDim2(1, 0, 1, 0),Data=self.images.gradientp90,Parent=notification.background,Transparency=0.5,ZIndex=(z + 1)});
			notification.accentBar = utility:Draw("Square", {Size=newUDim2(0, 5, 1, 4),Position=newUDim2(0, 0, 0, -2),Parent=notification.background,ThemeColor=(((color == nil) and "Accent") or ""),ZIndex=(z + 5)});
			notification.text = utility:Draw("Text", {Position=newUDim2(0, 13, 0, 2),ThemeColor="Primary Text",Text=message,Outline=true,Font=2,Size=13,ZIndex=(z + 4),Parent=notification.background});
			if color then
				notification.accentBar.Color = color;
			end
		end
		notification.Remove = function(self)
		...
		task.spawn(function()
			...
			utility:Connection(utility:Tween(notification.background, "Position", newUDim2(0, -500, 0, 0), 0.25).Completed, (function()
				notification:Remove();
			end));
		end);
	end;
	self.UpdateNotifications = function(self)
		local i = 0;
	...
	self.NewIndicator = function(data)
		local indicator = {title=(data.title or "indicator"),enabled=(data.enabled or false),position=(data.position or newUDim2(0, 15, 0, 300)),values={},objects={valueObjects={}},spacing="   "};
		table.insert(self.indicators, indicator);
		do
			local z = self.zindexOrder.indicator;
			local objs = indicator.objects;
			objs.background = utility:Draw("Square", {Size=newUDim2(0, 200, 0, 16),Position=indicator.position,ThemeColor="Background",ZIndex=z});
			objs.border1 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Border 2",Parent=objs.background,ZIndex=(z - 1)});
			objs.border2 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Border 3",Parent=objs.border1,ZIndex=(z - 2)});
			objs.topborder = utility:Draw("Square", {Size=newUDim2(1, 0, 0, 1),ThemeColor="Accent",Parent=objs.background,ZIndex=(z + 1)});
			objs.textlabel = utility:Draw("Text", {Position=newUDim2(0.5, 0, 0, 1),ThemeColor="Primary Text",Text=indicator.title,Size=13,Font=2,ZIndex=(z + 2),Center=true,Outline=true,Parent=objs.background});
		end
		indicator.Update = function(self)
			local xSize = 125;
			local yPos = 0;
		...
		indicator.AddValue = function(self, data)
			local value = {key=(data.key or ""),value=(data.value or ""),order=(data.order or (#self.values + 1)),enabled=(((data.enabled == nil) and true) or data.enabled),objects={}};
			table.insert(self.values, value);
			do
				local z = library.zindexOrder.indicator;
				local objs = value.objects;
				objs.background = utility:Draw("Square", {Size=newUDim2(1, 0, 0, 16),ThemeColor="Background",ZIndex=z,Parent=indicator.objects.background});
				objs.border1 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Border 2",Parent=objs.background,ZIndex=(z - 1)});
				objs.border2 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Border 3",Parent=objs.border1,ZIndex=(z - 2)});
				objs.keyLabel = utility:Draw("Text", {Position=newUDim2(0, 3, 0, 1),ThemeColor="Option Text 2",Size=13,Font=2,ZIndex=(z + 2),Outline=true,Parent=objs.background});
				objs.valueLabel = utility:Draw("Text", {Position=newUDim2(0, 0, 0, 1),ThemeColor="Option Text 2",Size=13,Font=2,ZIndex=(z + 2),Outline=true,Parent=objs.background});
			end
			value.Remove = function(self)
				table.remove(indicator.values, table.find(indicator.values, value));
				self.objects.background:Remove();
				table.clear(self);
				indicator:Update();
			end;
			value.SetEnabled = function(self, bool)
				if (typeof(bool) == "boolean") then
					self.enabled = bool;
					indicator:Update();
				end
			end;
			value.SetValue = function(self, str)
				if (typeof(str) == "string") then
					self.value = str;
					indicator:Update();
				end
			end;
			value.SetKey = function(self, str)
				if (typeof(str) == "string") then
					self.key = str;
					indicator:Update();
				end
		...
		indicator.GetValue = function(self, idx)
		...
		indicator.SetEnabled = function(self, bool)
			if (typeof(bool) == "boolean") then
				self.enabled = bool;
		...
		indicator.SetPosition = function(self, udim2)
	...
	self.NewWindow = function(data)
		local window = {title=(data.title or ""),selectedTab=nil,tabs={},objects={},colorpicker={objects={},color=c3new(1, 0, 0),trans=0},dropdown={objects={values={}},max=5}};
		...
		do
			...
			objs.background = utility:Draw("Square", {Size=size,Position=position,ThemeColor="Background",ZIndex=z});
			objs.innerBorder1 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Border 3",ZIndex=(z - 1),Parent=objs.background});
			objs.innerBorder2 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Border 1",ZIndex=(z - 2),Parent=objs.innerBorder1});
			objs.midBorder = utility:Draw("Square", {Size=newUDim2(1, 10, 1, 25),Position=newUDim2(0, -5, 0, -20),ThemeColor="Border 2",ZIndex=(z - 3),Parent=objs.innerBorder2});
			objs.outerBorder1 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Border 1",ZIndex=(z - 4),Parent=objs.midBorder});
			objs.outerBorder2 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Border 3",ZIndex=(z - 5),Parent=objs.outerBorder1});
			objs.topBorder = utility:Draw("Square", {Size=newUDim2(1, 0, 0, 1),ThemeColor="Accent",ZIndex=(z + 1),Parent=objs.background});
			objs.title = utility:Draw("Text", {Position=newUDim2(0, 7, 0, 2),ThemeColor="Primary Text",Text=window.title,Font=2,Size=13,ZIndex=(z + 1),Outline=true,Parent=objs.midBorder});
			objs.groupBackground = utility:Draw("Square", {Size=newUDim2(1, -16, 1, -(16 + 23)),Position=newUDim2(0, 8, 0, 8 + 23),ThemeColor="Group Background",ZIndex=(z + 5),Parent=objs.background});
			objs.groupInnerBorder = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Border 1",ZIndex=(z + 4),Parent=objs.groupBackground});
			objs.groupOuterBorder = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Border 3",ZIndex=(z + 3),Parent=objs.groupInnerBorder});
			objs.tabHolder = utility:Draw("Square", {Size=newUDim2(1, 0, 0, 20),Position=newUDim2(0, 0, 0, -21),Parent=objs.groupBackground,Transparency=0,ZIndex=(z + 1)});
			objs.columnholder1 = utility:Draw("Square", {Size=newUDim2(0.48, 0, 0.96, 0),Position=newUDim2(0.01, 0, 0.02, 0),Transparency=0,ZIndex=(z + 6),Parent=objs.groupBackground});
			objs.columnholder2 = utility:Draw("Square", {Size=newUDim2(0.48, 0, 0.96, 0),Position=newUDim2(1 - (0.48 + 0.01), 0, 0.02, 0),Transparency=0,ZIndex=(z + 6),Parent=objs.groupBackground});
			objs.dragdetector = utility:Draw("Square", {Size=newUDim2(1, 0, 1, 0),Parent=objs.midBorder,Transparency=0,ZIndex=(z + 2)});
		...
		do
			do
				...
				objs.background = utility:Draw("Square", {Visible=false,Size=newUDim2(0, 200, 0, 242),Position=newUDim2(1, -200, 1, 10),ThemeColor="Background",ZIndex=z,Parent=window.objects.background});
				objs.border1 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Border",ZIndex=(z - 1),Parent=objs.background});
				objs.border2 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Border 1",ZIndex=(z - 2),Parent=objs.border1});
				objs.border3 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Border",ZIndex=(z - 3),Parent=objs.border2});
				objs.statusText = utility:Draw("Text", {Position=newUDim2(0, 5, 0, 4),Text="colorpicker_status_text",ThemeColor="Option Text 1",Size=13,Font=2,Outline=true,ZIndex=(z + 1),Parent=objs.background});
				objs.mainColor = utility:Draw("Square", {Size=newUDim2(0, 175, 0, 175),Position=newUDim2(0, 5, 0, 25),Color=c3new(1, 0, 0),ZIndex=(z + 2),Parent=objs.background});
				objs.sat1 = utility:Draw("Image", {Size=newUDim2(1, 0, 1, 0),Data=crypt.base64decode(""),ZIndex=(z + 3),Parent=objs.mainColor});
				objs.sat2 = utility:Draw("Image", {Size=newUDim2(1, 0, 1, 0),Data=crypt.base64decode(""),ZIndex=(z + 4),Parent=objs.mainColor});
				objs.colorBorder = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Border",ZIndex=(z + 1),Parent=objs.mainColor});
				objs.mainDetector = utility:Draw("Square", {Size=newUDim2(1, 0, 1, 0),Transparency=0,ZIndex=(z + 10),Parent=objs.mainColor});
				objs.hue = utility:Draw("Image", {Size=newUDim2(0, 175, 0, 10),Position=newUDim2(0, 5, 0, 205),Data=library.images.colorhue,ZIndex=(z + 2),Parent=objs.background});
				objs.hueBorder = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Border",ZIndex=(z + 1),Parent=objs.hue});
				objs.hueDetector = utility:Draw("Square", {Size=newUDim2(1, 0, 1, 0),Transparency=0,ZIndex=(z + 10),Parent=objs.hue});
				objs.transColor = utility:Draw("Square", {Size=newUDim2(0, 10, 0, 175),Position=newUDim2(0, 185, 0, 25),Color=c3new(1, 0, 0),ZIndex=(z + 2),Parent=objs.background});
				objs.trans = utility:Draw("Image", {Size=newUDim2(1, 0, 1, 0),Data=library.images.colortrans,ZIndex=(z + 3),Parent=objs.transColor});
				objs.transBorder = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Border",ZIndex=(z + 1),Parent=objs.transColor});
				objs.transDetector = utility:Draw("Square", {Size=newUDim2(1, 0, 1, 0),Transparency=0,ZIndex=(z + 10),Parent=objs.transColor});
				objs.pointer = utility:Draw("Square", {Size=newUDim2(0, 2, 0, 2),Position=newUDim2(0, 0, 0, 0),Color=c3new(1, 1, 1),ZIndex=(z + 6),Parent=objs.mainColor});
				objs.pointerBorder = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),Color=c3new(0, 0, 0),ZIndex=(z + 5),Parent=objs.pointer});
				objs.hueSlider = utility:Draw("Square", {Size=newUDim2(0, 1, 1, 0),Color=c3new(1, 1, 1),ZIndex=(z + 4),Parent=objs.hue});
				objs.hueSliderBorder = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),Color=c3new(0, 0, 0),ZIndex=(z + 3),Parent=objs.hueSlider});
				objs.transSlider = utility:Draw("Square", {Size=newUDim2(1, 0, 0, 1),Color=c3new(1, 1, 1),ZIndex=(z + 5),Parent=objs.trans});
				objs.transSliderBorder = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),Color=c3new(0, 0, 0),ZIndex=(z + 4),Parent=objs.transSlider});
				objs.rBackground = utility:Draw("Square", {Size=newUDim2(0, 60, 0, 15),Position=newUDim2(0, 5, 1, -20),ThemeColor="Option Background",Parent=objs.background,ZIndex=(z + 5)});
				objs.rBorder = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),Color=c3new(0, 0, 0),ZIndex=(z + 4),Parent=objs.rBackground});
				objs.rText = utility:Draw("Text", {Position=newUDim2(0.5, 0, 0, 0),Color=c3new(1, 0.1, 0.1),Text="R",Size=13,Font=2,Outline=true,Center=true,ZIndex=(z + 6),Parent=objs.rBackground});
				objs.gBackground = utility:Draw("Square", {Size=newUDim2(0, 60, 0, 15),Position=newUDim2(0, 70, 1, -20),ThemeColor="Option Background",Parent=objs.background,ZIndex=(z + 5)});
				objs.gBorder = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),Color=c3new(0, 0, 0),ZIndex=(z + 4),Parent=objs.gBackground});
				objs.gText = utility:Draw("Text", {Position=newUDim2(0.5, 0, 0, 0),Color=c3new(0.1, 1, 0.1),Text="G",Size=13,Font=2,Outline=true,Center=true,ZIndex=(z + 6),Parent=objs.gBackground});
				objs.bBackground = utility:Draw("Square", {Size=newUDim2(0, 60, 0, 15),Position=newUDim2(0, 135, 1, -20),ThemeColor="Option Background",Parent=objs.background,ZIndex=(z + 5)});
				objs.bBorder = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),Color=c3new(0, 0, 0),ZIndex=(z + 4),Parent=objs.bBackground});
				objs.bText = utility:Draw("Text", {Position=newUDim2(0.5, 0, 0, 0),Color=c3new(0.1, 0.1, 1),Text="B",Size=13,Font=2,Outline=true,Center=true,ZIndex=(z + 6),Parent=objs.bBackground});
			...
			window.colorpicker.Visualize = function(self, c3, a)
		...
		do
			do
				...
				objs.background = utility:Draw("Square", {Visible=false,Size=newUDim2(1, -3, 0, 50),Position=newUDim2(0, 3, 1, 0),ThemeColor="Background",ZIndex=z,Parent=window.objects.background});
				objs.border1 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Border",ZIndex=(z - 1),Parent=objs.background});
				objs.border2 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Border 1",ZIndex=(z - 2),Parent=objs.border1});
				objs.border3 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Border",ZIndex=(z - 3),Parent=objs.border2});
			end
			window.dropdown.Refresh = function(self)
				if (self.selected ~= nil) then
					...
					for idx, value in next, list.values do
						...
						if (valueObject == nil) then
							valueObject = {};
							valueObject.background = utility:Draw("Square", {Size=newUDim2(1, -4, 0, 18),Color=Color3.new(0.25, 0.25, 0.25),Transparency=0,ZIndex=(library.zindexOrder.dropdown + 1),Parent=self.objects.background});
							valueObject.text = utility:Draw("Text", {Position=newUDim2(0, 3, 0, 1),ThemeColor="Option Text 2",Text=tostring(value),Size=13,Font=2,ZIndex=(library.zindexOrder.dropdown + 2),Parent=valueObject.background});
		...
		window.AddTab = function(self, text, order)
			local tab = {text=text,order=(order or (#self.tabs + 1)),callback=function()
			end,objects={},sections={}};
			table.insert(self.tabs, tab);
			do
				local objs = tab.objects;
				local z = library.zindexOrder.window + 5;
				objs.background = utility:Draw("Square", {Size=newUDim2(0, 50, 1, 0),Parent=self.objects.tabHolder,ThemeColor="Unselected Tab Background",ZIndex=z});
				objs.innerBorder = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Border 1",ZIndex=(z - 1),Parent=objs.background});
				objs.outerBorder = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Border 3",ZIndex=(z - 2),Parent=objs.innerBorder});
				objs.topBorder = utility:Draw("Square", {Size=newUDim2(1, 0, 0, 1),ThemeColor="Unselected Tab Background",ZIndex=(z + 1),Parent=objs.background});
				objs.text = utility:Draw("Text", {ThemeColor="Unselected Tab Text",Text=text,Size=13,Font=2,ZIndex=(z + 1),Outline=true,Center=true,Parent=objs.background});
				utility:Connection(objs.background.MouseButton1Down, function()
					tab:Select();
				end);
			end
			tab.AddSection = function(self, text, side, order)
				local section = {text=tostring(text),side=(((side == nil) and 1) or clamp(side, 1, 2)),order=(order or (#self.sections + 1)),enabled=true,objects={},options={}};
				table.insert(self.sections, section);
				do
					local objs = section.objects;
					local z = library.zindexOrder.window + 15;
					objs.background = utility:Draw("Square", {ThemeColor="Section Background",ZIndex=z,Parent=window.objects["columnholder" .. section.side]});
					objs.innerBorder = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 1),Position=newUDim2(0, -1, 0, 0),ThemeColor="Border 3",ZIndex=(z - 1),Parent=objs.background});
					objs.outerBorder = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 1),Position=newUDim2(0, -1, 0, 0),ThemeColor="Border 1",ZIndex=(z - 2),Parent=objs.innerBorder});
					objs.topBorder1 = utility:Draw("Square", {Size=newUDim2(0.025, 1, 0, 1),Position=newUDim2(0, -1, 0, 0),ThemeColor="Accent",ZIndex=(z + 1),Parent=objs.background});
					objs.topBorder2 = utility:Draw("Square", {ThemeColor="Accent",ZIndex=(z + 1),Parent=objs.background});
					objs.textlabel = utility:Draw("Text", {Position=newUDim2(0.0425, 0, 0, -7),ThemeColor="Primary Text",Size=13,Font=2,ZIndex=(z + 1),Parent=objs.background});
					objs.optionholder = utility:Draw("Square", {Size=newUDim2(1 - 0.03, 0, 1, -15),Position=newUDim2(0.015, 0, 0, 13),Transparency=0,ZIndex=(z + 1),Parent=objs.background});
				end
				section.SetText = function(self, text)
				...
				section.UpdateOptions = function(self)
				...
				section.SetEnabled = function(self, bool)
					if (typeof(bool) == "boolean") then
				...
				section.AddToggle = function(self, data)
					local toggle = {class="toggle",flag=data.flag,text="",tooltip="",order=(#self.options + 1),state=false,risky=false,callback=function()
					end,enabled=true,options={},objects={}};
					...
					do
						...
						objs.holder = utility:Draw("Square", {Size=newUDim2(1, 0, 0, 17),Transparency=0,ZIndex=(z + 5),Parent=section.objects.optionholder});
						objs.background = utility:Draw("Square", {Size=newUDim2(0, 8, 0, 8),Position=newUDim2(0, 2, 0, 4),ThemeColor="Option Background",ZIndex=(z + 3),Parent=objs.holder});
						objs.gradient = utility:Draw("Image", {Size=newUDim2(1, 0, 1, 0),Data=library.images.gradientp45,Transparency=0.25,ZIndex=(z + 4),Parent=objs.background});
						objs.border1 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Option Border 1",ZIndex=(z + 2),Parent=objs.background});
						objs.border2 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Option Border 2",ZIndex=(z + 1),Parent=objs.border1});
						objs.text = utility:Draw("Text", {Position=newUDim2(0, 19, 0, 1),ThemeColor="Option Text 3",Size=13,Font=2,ZIndex=(z + 1),Outline=true,Parent=objs.holder});
					...
					toggle.SetState = function(self, bool, nocallback)
						if (typeof(bool) == "boolean") then
							self.state = bool;
							if self.flag then
					...
					toggle.SetText = function(self, str)
						if (typeof(str) == "string") then
							self.text = str;
							self.objects.text.Text = str;
						end
					end;
					toggle.UpdateOptions = function(self)
						table.sort(self.options, function(a, b)
							return a.order < b.order;
						end);
						local x, y = 0, 0;
					...
					toggle.AddColor = function(self, data)
						local color = {class="color",flag=data.flag,text="",tooltip="",order=(#self.options + 1),callback=function()
						end,color=Color3.new(1, 1, 1),trans=0,open=false,enabled=true,objects={}};
						local blacklist = {"objects"};
						for i, v in next, data do
						...
						do
							...
							local z = library.zindexOrder.window + 25;
							objs.holder = utility:Draw("Square", {Size=newUDim2(0, 21, 0, 17),Transparency=0,ZIndex=(z + 6),Parent=self.objects.holder});
							objs.background = utility:Draw("Square", {Size=newUDim2(0, 15, 0, 8),Position=newUDim2(0, 4, 0, 5),ZIndex=(z + 3),Parent=objs.holder});
							objs.gradient = utility:Draw("Image", {Size=newUDim2(1, 0, 1, 0),Data=library.images.gradientp45,Transparency=0.25,ZIndex=(z + 4),Parent=objs.background});
							objs.border1 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Option Border 1",ZIndex=(z + 2),Parent=objs.background});
							objs.border2 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Option Border 2",ZIndex=(z + 1),Parent=objs.border1});
							utility:Connection(objs.holder.MouseEnter, function()
						...
						color.SetColor = function(self, c3, nocallback)
							if (typeof(c3) == "Color3") then
								local h, s, v = c3:ToHSV();
								c3 = fromhsv(h, clamp(s, 0.005, 0.995), clamp(v, 0.005, 0.995));
								self.color = c3;
						...
						color.SetTrans = function(self, trans, nocallback)
						...
						color.SetOpen = function(self, bool)
							if (typeof(bool) == "boolean") then
								self.open = bool;
								if bool then
					...
					toggle.AddBind = function(self, data)
						local bind = {class="bind",flag=data.flag,text="",tooltip="",bind="none",mode="toggle",order=(#self.options + 1),callback=function()
						end,keycallback=function()
						end,indicatorValue=library.keyIndicator:AddValue({value="value",key="key",enabled=false}),noindicator=false,invertindicator=false,state=false,nomouse=false,enabled=true,binding=false,objects={}};
						local blacklist = {"objects"};
						for i, v in next, data do
						...
						do
							...
							local z = library.zindexOrder.window + 25;
							objs.holder = utility:Draw("Square", {Size=newUDim2(0, 0, 0, 17),Transparency=0,ZIndex=(z + 6),Parent=self.objects.holder});
							objs.keyText = utility:Draw("Text", {ThemeColor="Option Text 3",Size=13,Font=2,ZIndex=(z + 1),Parent=objs.holder});
							utility:Connection(objs.holder.MouseEnter, function()
						...
						local c;
						bind.SetBind = function(self, keybind)
							if c then
								c:Disconnect();
								if bind.flag then
						...
						bind.SetKeyText = function(self, str)
							str = tostring(str);
					...
					toggle.AddSlider = function(self, data)
						local slider = {class="slider",flag=data.flag,suffix="",tooltip="",order=(#self.options + 1),value=0,min=0,max=100,increment=1,callback=function()
						end,enabled=true,dragging=false,focused=false,objects={}};
						...
						do
							...
							local z = library.zindexOrder.window + 25;
							objs.holder = utility:Draw("Square", {Size=newUDim2(1, 0, 0, 20),Transparency=0,ZIndex=(z + 6),Parent=toggle.objects.holder});
							objs.background = utility:Draw("Square", {Size=newUDim2(1, -4, 1, -8),Position=newUDim2(0, 2, 0, 4),ThemeColor="Option Background",ZIndex=(z + 2),Parent=objs.holder});
							objs.slider = utility:Draw("Square", {Size=newUDim2(0, 0, 1, 0),ThemeColor="Accent",ZIndex=(z + 3),Parent=objs.background});
							objs.border1 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Option Border 1",ZIndex=(z + 1),Parent=objs.background});
							objs.border2 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Option Border 2",ZIndex=z,Parent=objs.border1});
							objs.gradient = utility:Draw("Image", {Size=newUDim2(1, 0, 1, 0),Data=library.images.gradientp90,Transparency=0.65,ZIndex=(z + 4),Parent=objs.background});
							objs.text = utility:Draw("Text", {Position=newUDim2(0.5, 0, 0, -1),ThemeColor="Option Text 3",Size=13,Font=2,ZIndex=(z + 5),Outline=true,Center=true,Parent=objs.background});
							utility:Connection(objs.holder.MouseEnter, function()
								objs.border1.ThemeColor = "Accent";
							end);
							utility:Connection(objs.holder.MouseLeave, function()
						...
						slider.SetValue = function(self, value, nocallback)
							if (typeof(value) == "number") then
								...
								utility:Tween(self.objects.slider, "Size", size, 0.05, Enum.EasingDirection.Out, Enum.EasingStyle.Quad);
								utility:Tween(self.objects.slider, "Position", pos, 0.05, Enum.EasingDirection.Out, Enum.EasingStyle.Quad);
								self.value = newValue;
					...
					toggle.AddList = function(self, data)
						local list = {class="list",flag=data.flag,text="",selected="",tooltip="",order=(#self.options + 1),callback=function()
						end,enabled=true,multi=false,open=false,values={},objects={}};
						...
						do
							...
							local z = library.zindexOrder.window + 25;
							objs.holder = utility:Draw("Square", {Size=newUDim2(1, 0, 0, 22),Transparency=0,ZIndex=(z + 6),Parent=toggle.objects.holder});
							objs.background = utility:Draw("Square", {Size=newUDim2(1, -4, 1, -8),Position=newUDim2(0, 2, 0, 4),ThemeColor="Option Background",ZIndex=(z + 2),Parent=objs.holder});
							objs.border1 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Option Border 1",ZIndex=(z + 1),Parent=objs.background});
							objs.border2 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Option Border 2",ZIndex=z,Parent=objs.border1});
							objs.gradient = utility:Draw("Image", {Size=newUDim2(1, 0, 1, 0),Data=library.images.gradientp90,Transparency=0.65,ZIndex=(z + 4),Parent=objs.background});
							objs.inputText = utility:Draw("Text", {Position=newUDim2(0, 4, 0, 0),ThemeColor="Option Text 2",Text="none",Size=13,Font=2,ZIndex=(z + 5),Outline=true,Parent=objs.background});
							objs.openText = utility:Draw("Text", {Position=newUDim2(1, -10, 0, 0),ThemeColor="Option Text 3",Text="+",Size=13,Font=2,ZIndex=(z + 5),Outline=true,Parent=objs.background});
							utility:Connection(objs.holder.MouseEnter, function()
								objs.border1.ThemeColor = "Accent";
							end);
							utility:Connection(objs.holder.MouseLeave, function()
						...
						list.Select = function(self, option, nocallback)
						...
						list.AddValue = function(self, value)
							table.insert(list.values, tostring(value));
							if (window.dropdown.selected == list) then
								window.dropdown:Refresh();
							end
						end;
						list.RemoveValue = function(self, value)
						...
						list.ClearValues = function(self)
				...
				section.AddSlider = function(self, data)
					local slider = {class="slider",flag=data.flag,text="",tooltip="",suffix="",order=(#self.options + 1),value=0,min=0,max=100,increment=1,callback=function()
					end,enabled=true,dragging=false,focused=false,risky=false,objects={}};
					local blacklist = {"objects","dragging"};
					for i, v in next, data do
						if (not table.find(blacklist, i) and (slider[i] ~= nil) and (typeof(slider[i]) == typeof(v))) then
							slider[i] = v;
					...
					do
						local objs = slider.objects;
						local z = library.zindexOrder.window + 25;
						objs.holder = utility:Draw("Square", {Size=newUDim2(1, 0, 0, 32),Transparency=0,ZIndex=(z + 4),Parent=section.objects.optionholder});
						objs.background = utility:Draw("Square", {Size=newUDim2(1, -4, 0, 11),Position=newUDim2(0, 2, 1, -14),ThemeColor="Option Background",ZIndex=(z + 2),Parent=objs.holder});
						objs.slider = utility:Draw("Square", {Size=newUDim2(0, 0, 1, 0),ThemeColor="Accent",ZIndex=(z + 3),Parent=objs.background});
						objs.border1 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Option Border 1",ZIndex=(z + 1),Parent=objs.background});
						objs.border2 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Option Border 2",ZIndex=z,Parent=objs.border1});
						objs.gradient = utility:Draw("Image", {Size=newUDim2(1, 0, 1, 0),Data=library.images.gradientp90,Transparency=0.65,ZIndex=(z + 4),Parent=objs.background});
						objs.text = utility:Draw("Text", {Position=newUDim2(0, 2, 0, 1),ThemeColor="Option Text 3",Size=13,Font=2,ZIndex=(z + 1),Outline=true,Parent=objs.holder});
						objs.plusDetector = utility:Draw("Square", {Size=newUDim2(0, 14, 0, 14),Position=newUDim2(1, -28, 0, 1),Transparency=0,ZIndex=(z + 5),Parent=objs.holder});
						objs.minusDetector = utility:Draw("Square", {Size=newUDim2(0, 14, 0, 14),Position=newUDim2(1, -14, 0, 1),Transparency=0,ZIndex=(z + 5),Parent=objs.holder});
						objs.plusText = utility:Draw("Text", {Position=newUDim2(0.5, 0, 0, -1),ThemeColor="Option Text 3",Text="+",Size=13,Font=2,ZIndex=(z + 4),Center=true,Outline=true,Parent=objs.plusDetector});
						objs.minusText = utility:Draw("Text", {Position=newUDim2(0.5, 0, 0, -1),ThemeColor="Option Text 3",Text="-",Size=13,Font=2,ZIndex=(z + 4),Center=true,Outline=true,Parent=objs.minusDetector});
						utility:Connection(objs.holder.MouseEnter, function()
							objs.border1.ThemeColor = "Accent";
						end);
						utility:Connection(objs.holder.MouseLeave, function()
					...
					slider.SetText = function(self, str)
						if (typeof(str) == "string") then
							self.text = str;
				...
				section.AddButton = function(self, data)
					local button = {class="button",flag=data.flag,text="",suffix="",tooltip="",order=(#self.options + 1),callback=function()
					end,confirm=false,enabled=true,risky=false,objects={},subbuttons={}};
					local blacklist = {"objects"};
					for i, v in next, data do
					...
					do
						...
						local z = library.zindexOrder.window + 25;
						objs.holder = utility:Draw("Square", {Size=newUDim2(1, 0, 0, 22),Transparency=0,ZIndex=(z + 4),Parent=section.objects.optionholder});
						objs.background = utility:Draw("Square", {Size=newUDim2(1, -4, 0, 14),Position=newUDim2(0, 2, 0, 4),ThemeColor="Option Background",ZIndex=(z + 2),Parent=objs.holder});
						objs.border1 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Option Border 1",ZIndex=(z + 1),Parent=objs.background});
						objs.border2 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Option Border 2",ZIndex=z,Parent=objs.border1});
						objs.gradient = utility:Draw("Image", {Size=newUDim2(1, 0, 1, 0),Data=library.images.gradientp90,Transparency=0.65,ZIndex=(z + 3),Parent=objs.background});
						objs.text = utility:Draw("Text", {Position=newUDim2(0.5, 0, 0, 0),ThemeColor="Option Text 3",Size=13,Font=2,ZIndex=(z + 4),Outline=true,Center=true,Parent=objs.background});
						utility:Connection(objs.holder.MouseEnter, function()
							objs.border1.ThemeColor = "Accent";
						end);
						utility:Connection(objs.holder.MouseLeave, function()
					...
					button.AddButton = function(self, data)
						local button = {class="button",flag=data.flag,text="",suffix="",tooltip="",order=(#self.subbuttons + 1),callback=function()
						end,confirm=false,enabled=true,objects={}};
						local blacklist = {"objects"};
						for i, v in next, data do
							if (not table.find(blacklist, i) and (button[i] ~= nil)) then
								button[i] = v;
						...
						do
							local objs = button.objects;
							local z = library.zindexOrder.window + 25;
							objs.holder = utility:Draw("Square", {Size=newUDim2(1, 0, 1, 0),Transparency=0,ZIndex=(z + 5),Parent=self.objects.holder});
							objs.background = utility:Draw("Square", {Size=newUDim2(1, -4, 1, -8),Position=newUDim2(0, 2, 0, 4),ThemeColor="Option Background",ZIndex=(z + 2),Parent=objs.holder});
							objs.border1 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Option Border 1",ZIndex=(z + 1),Parent=objs.background});
							objs.border2 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Option Border 2",ZIndex=z,Parent=objs.border1});
							objs.gradient = utility:Draw("Image", {Size=newUDim2(1, 0, 1, 0),Data=library.images.gradientp90,Transparency=0.65,ZIndex=(z + 3),Parent=objs.background});
					...
					button.UpdateOptions = function(self)
						...
						for i, v in next, self.subbuttons do
							...
							v.objects.holder.Position = newUDim2((i * buttonSize) + 0.01, 0, 0, 0);
						end
					end;
					button.SetText = function(self, str)
						if (typeof(str) == "string") then
				...
				section.AddSeparator = function(self, data)
					local separator = {class="separator",flag=data.flag,text="",order=(#self.options + 1),enabled=true,objects={}};
					local blacklist = {"objects","dragging"};
					for i, v in next, data do
					...
					do
						...
						local z = library.zindexOrder.window + 25;
						objs.holder = utility:Draw("Square", {Size=newUDim2(1, 0, 0, 18),Transparency=0,ZIndex=z,Parent=section.objects.optionholder});
						objs.line1 = utility:Draw("Square", {Position=newUDim2(0, 0, 0, 1),ThemeColor="Option Background",ZIndex=(z + 1),Parent=objs.holder});
						objs.line2 = utility:Draw("Square", {Position=newUDim2(0, 0, 0, 1),ThemeColor="Option Background",ZIndex=(z + 1),Parent=objs.holder});
						objs.border1 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Option Border 2",ZIndex=z,Parent=objs.line1});
						objs.border2 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Option Border 2",ZIndex=z,Parent=objs.line2});
						objs.text = utility:Draw("Text", {Position=newUDim2(0.5, 0, 0, 1),ThemeColor="Option Text 2",Size=13,Font=2,ZIndex=z,Outline=true,Center=true,Parent=objs.holder});
					end
					separator.SetText = function(self, str)
						if (typeof(str) == "string") then
							self.text = str;
							self.objects.text.Text = str;
							local xScale = ((1 - utility:ConvertNumberRange(self.objects.text.TextBounds.X, 0, self.objects.holder.Object.Size.X, 0, 1)) / 2) - (((str == "") and 0) or 0.04);
				...
				section.AddColor = function(self, data)
					local color = {class="color",flag=data.flag,text="",tooltip="",order=(#self.options + 1),callback=function()
					end,color=Color3.new(1, 1, 1),trans=0,open=false,enabled=true,risky=false,objects={}};
					local blacklist = {"objects"};
					for i, v in next, data do
						if (not table.find(blacklist, i) and (color[i] ~= nil)) then
							color[i] = v;
					...
					do
						local objs = color.objects;
						local z = library.zindexOrder.window + 25;
						objs.holder = utility:Draw("Square", {Size=newUDim2(1, 0, 0, 19),Transparency=0,ZIndex=(z + 5),Parent=section.objects.optionholder});
						objs.background = utility:Draw("Square", {Size=newUDim2(0, 15, 0, 8),Position=newUDim2(1, -16, 0, 5),ZIndex=(z + 3),Parent=objs.holder});
						objs.gradient = utility:Draw("Image", {Size=newUDim2(1, 0, 1, 0),Data=library.images.gradientp45,Transparency=0.25,ZIndex=(z + 4),Parent=objs.background});
						objs.border1 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Option Border 1",ZIndex=(z + 2),Parent=objs.background});
						objs.border2 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Option Border 2",ZIndex=(z + 1),Parent=objs.border1});
						objs.text = utility:Draw("Text", {Position=newUDim2(0, 2, 0, 2),ThemeColor=((color.risky and "Risky Text Enabled") or "Option Text 3"),Size=13,Font=2,ZIndex=(z + 1),Outline=true,Parent=objs.holder});
						utility:Connection(objs.holder.MouseEnter, function()
							objs.border1.ThemeColor = "Accent";
						end);
						utility:Connection(objs.holder.MouseLeave, function()
					...
					color.SetText = function(self, str)
						if (typeof(str) == "string") then
							self.text = str;
							self.objects.text.Text = str;
						end
				...
				section.AddBox = function(self, data)
					local box = {class="box",flag=data.flag,text="",input="",order=(#self.options + 1),callback=function()
					end,enabled=true,focused=false,risky=false,objects={}};
					local blacklist = {"objects","dragging"};
					for i, v in next, data do
					...
					do
						...
						local z = library.zindexOrder.window + 25;
						objs.holder = utility:Draw("Square", {Size=newUDim2(1, 0, 0, 37),Transparency=0,ZIndex=(z + 4),Parent=section.objects.optionholder});
						objs.background = utility:Draw("Square", {Size=newUDim2(1, -4, 0, 15),Position=newUDim2(0, 2, 1, -17),ThemeColor="Option Background",ZIndex=(z + 2),Parent=objs.holder});
						objs.border1 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Option Border 1",ZIndex=(z + 1),Parent=objs.background});
						objs.border2 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Option Border 2",ZIndex=z,Parent=objs.border1});
						objs.gradient = utility:Draw("Image", {Size=newUDim2(1, 0, 1, 0),Data=library.images.gradientp90,Transparency=0.65,ZIndex=(z + 4),Parent=objs.background});
						objs.text = utility:Draw("Text", {Position=newUDim2(0, 2, 0, 2),ThemeColor=((box.risky and "Risky Text Enabled") or "Option Text 2"),Size=13,Font=2,ZIndex=(z + 1),Outline=true,Parent=objs.holder});
						objs.inputText = utility:Draw("Text", {Position=newUDim2(0, 2, 0, 0),ThemeColor="Option Text 2",Size=13,Font=2,ZIndex=(z + 5),Outline=true,Parent=objs.background});
						utility:Connection(objs.holder.MouseEnter, function()
							objs.border1.ThemeColor = "Accent";
						end);
						utility:Connection(objs.holder.MouseLeave, function()
					...
					box.SetText = function(self, str)
						if (typeof(str) == "string") then
							self.text = str;
							self.objects.text.Text = str;
						end
					end;
					box.SetInput = function(self, str, nocallback)
						if (typeof(str) == "string") then
							self.input = str;
					...
					box.CaptureFocus = function(self, clear)
						box.focused = true;
						if clear then
							input = "";
						end
					...
					box.ReleaseFocus = function(self, apply)
						box.focused = false;
				...
				section.AddBind = function(self, data)
					local bind = {class="bind",flag=data.flag,text="",tooltip="",bind="none",mode="toggle",order=(#self.options + 1),callback=function()
					end,keycallback=function()
					end,indicatorValue=library.keyIndicator:AddValue({value="value",key="key",enabled=false}),noindicator=false,state=false,nomouse=false,enabled=true,binding=false,risky=false,objects={}};
					local blacklist = {"objects"};
					for i, v in next, data do
						if (not table.find(blacklist, i) and (bind[i] ~= nil)) then
							bind[i] = v;
					...
					do
						...
						local z = library.zindexOrder.window + 25;
						objs.holder = utility:Draw("Square", {Size=newUDim2(1, 0, 0, 19),Transparency=0,ZIndex=(z + 5),Parent=section.objects.optionholder});
						objs.text = utility:Draw("Text", {Position=newUDim2(0, 2, 0, 2),ThemeColor=((bind.risky and "Risky Text") or "Option Text 2"),Size=13,Font=2,ZIndex=(z + 1),Outline=true,Parent=objs.holder});
						objs.keyText = utility:Draw("Text", {ThemeColor="Option Text 3",Size=13,Font=2,ZIndex=(z + 1),Parent=objs.holder});
						utility:Connection(objs.holder.MouseEnter, function()
							objs.keyText.ThemeColor = "Accent";
						end);
					...
					local c;
					bind.SetText = function(self, str)
						if (typeof(str) == "string") then
							self.text = str;
							self.objects.text.Text = str;
				...
				section.AddList = function(self, data)
					local list = {class="list",flag=data.flag,text="",selected="",tooltip="",order=(#self.options + 1),callback=function()
					end,enabled=true,multi=false,open=false,risky=false,values={},objects={}};
					table.insert(self.options, list);
					local blacklist = {"objects"};
					for i, v in next, data do
						if ((not table.find(blacklist, i) ~= list[i]) ~= nil) then
					...
					do
						local objs = list.objects;
						local z = library.zindexOrder.window + 25;
						objs.holder = utility:Draw("Square", {Size=newUDim2(1, 0, 0, 40),Transparency=0,ZIndex=(z + 4),Parent=section.objects.optionholder});
						objs.background = utility:Draw("Square", {Size=newUDim2(1, -4, 0, 15),Position=newUDim2(0, 2, 1, -19),ThemeColor="Option Background",ZIndex=(z + 2),Parent=objs.holder});
						objs.border1 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Option Border 1",ZIndex=(z + 1),Parent=objs.background});
						objs.border2 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Option Border 2",ZIndex=z,Parent=objs.border1});
						objs.gradient = utility:Draw("Image", {Size=newUDim2(1, 0, 1, 0),Data=library.images.gradientp90,Transparency=0.65,ZIndex=(z + 4),Parent=objs.background});
						objs.text = utility:Draw("Text", {Position=newUDim2(0, 2, 0, 2),ThemeColor=((list.risky and "Risky Text Enabled") or "Option Text 2"),Size=13,Font=2,ZIndex=(z + 1),Outline=true,Parent=objs.holder});
						objs.inputText = utility:Draw("Text", {Position=newUDim2(0, 4, 0, 0),ThemeColor="Option Text 2",Text="none",Size=13,Font=2,ZIndex=(z + 5),Outline=true,Parent=objs.background});
						objs.openText = utility:Draw("Text", {Position=newUDim2(1, -10, 0, 0),ThemeColor="Option Text 3",Text="+",Size=13,Font=2,ZIndex=(z + 5),Outline=true,Parent=objs.background});
						utility:Connection(objs.holder.MouseEnter, function()
							objs.border1.ThemeColor = "Accent";
					...
					list.SetText = function(self, str)
						if (typeof(str) == "string") then
							self.text = str;
							self.objects.text.Text = str;
						end
				...
				section.AddText = function(self, data)
					local text = {class="text",flag=data.flag,text="",tooltip="",order=(#self.options + 1),enabled=true,risky=false,objects={}};
					local blacklist = {"objects"};
					for i, v in next, data do
					...
					do
						...
						local z = library.zindexOrder.window + 25;
						objs.holder = utility:Draw("Square", {Transparency=0,ZIndex=(z + 5),Parent=section.objects.optionholder});
						objs.text = utility:Draw("Text", {Position=newUDim2(0, 2, 0, 2),ThemeColor=((text.risky and "Risky Text Enabled") or "Option Text 2"),Size=13,Font=2,ZIndex=(z + 1),Outline=true,Parent=objs.holder});
					end
					text.SetText = function(self, str)
						if (typeof(str) == "string") then
							self.text = str;
							self.objects.text.Text = str;
			...
			tab.UpdateSections = function(self)
	...
	do
		...
		tooltipObjects.background = utility:Draw("Square", {ThemeColor="Group Background",ZIndex=z,Visible=false});
		tooltipObjects.border1 = utility:Draw("Square", {Size=UDim2.new(1, 2, 1, 2),Position=UDim2.new(0, -1, 0, -1),ThemeColor="Border 1",ZIndex=(z - 1),Parent=tooltipObjects.background});
		tooltipObjects.border2 = utility:Draw("Square", {Size=UDim2.new(1, 4, 1, 4),Position=UDim2.new(0, -2, 0, -2),ThemeColor="Border 3",ZIndex=(z - 2),Parent=tooltipObjects.background});
		tooltipObjects.text = utility:Draw("Text", {Position=UDim2.new(0, 3, 0, 0),ThemeColor="Primary Text",Size=13,Font=2,ZIndex=(z + 1),Outline=true,Parent=tooltipObjects.background});
		tooltipObjects.riskytext = utility:Draw("Text", {Position=UDim2.new(0, 3, 0, 0),ThemeColor="Risky Text Enabled",Text="[RISKY]",Size=13,Font=2,ZIndex=(z + 1),Outline=true,Parent=tooltipObjects.background});
	end
	do
		self.watermark = {objects={},text={{"chinaware.lol",true},{("V" .. getgenv().Config.Version),true},{getgenv().luaguardvars.DiscordName,true},{"0 fps",true},{"0ms",true}},lock="custom",position=newUDim2(0, 0, 0, 0),refreshrate=25};
		self.watermark.Update = function(self)
			...
			if library.flags.watermark_enabled then
				...
				self.position = ((self.lock == "Top Right") and newUDim2(0, (screensize.X - size.X) - 15, 0, 15)) or ((self.lock == "Top Left") and newUDim2(0, 15, 0, 15)) or ((self.lock == "Bottom Right") and newUDim2(0, (screensize.X - size.X) - 15, 0, (screensize.Y - size.Y) - 15)) or ((self.lock == "Bottom Left") and newUDim2(0, 15, 0, (screensize.Y - size.Y) - 15)) or ((self.lock == "Top") and newUDim2(0, (screensize.X / 2) - (size.X / 2), 0, 15)) or newUDim2(library.flags.watermark_x / 100, 0, library.flags.watermark_y / 100, 0);
				self.objects.background.Position = self.position;
			end
		end;
		do
			...
			objs.background = utility:Draw("Square", {Visible=false,Size=newUDim2(0, 200, 0, 17),Position=newUDim2(0, 800, 0, 100),ThemeColor="Background",ZIndex=z});
			objs.border1 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Border 2",Parent=objs.background,ZIndex=(z - 1)});
			objs.border2 = utility:Draw("Square", {Size=newUDim2(1, 2, 1, 2),Position=newUDim2(0, -1, 0, -1),ThemeColor="Border 3",Parent=objs.border1,ZIndex=(z - 2)});
			objs.topbar = utility:Draw("Square", {Size=newUDim2(1, 0, 0, 1),ThemeColor="Accent",ZIndex=(z + 1),Parent=objs.background});
			objs.text = utility:Draw("Text", {Position=newUDim2(0.5, 0, 0, 2),ThemeColor="Primary Text",Text="Watermark Text",Size=13,Font=2,ZIndex=(z + 1),Outline=true,Center=true,Parent=objs.background});
		end
	end
	local lasttick = tick();
...
library.CreateSettingsTab = function(self, menu)
	...
	creditsSection:AddText({text="kinija_t"});
	creditsSection:AddText({text="cursynwashere"});
	...
	mainSection:AddButton({text="Join Discord",flag="joindiscord",confirm=true,callback=function()
		local res = syn.request({Url="http://127.0.0.1:6463/rpc?v=1",Method="POST",Headers={["Content-Type"]="application/json",Origin="https://discord.com"},Body=game:GetService("HttpService"):JSONEncode({cmd="INVITE_BROWSER",nonce=game:GetService("HttpService"):GenerateGUID(false),args={code=getgenv().Config.Invite}})});
	end});
	...
	mainSection:AddSlider({text="Position Y",flag="keybind_indicator_y",min=0,max=100,increment=0.1,value=35,callback=function()
		library.keyIndicator:SetPosition(newUDim2(library.flags.keybind_indicator_x / 100, 0, library.flags.keybind_indicator_y / 100, 0));
	end});
